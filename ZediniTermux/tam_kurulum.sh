...
void main() {
    ...
    char* msg = "ZEDINI OS: TEMEL SAGLAM!";
    int i = 0;
    while(msg[i] != 0) {
        vidptr[i*2] = msg[i];
        vidptr[i*2+1] = 0x0A; // Yesil
        i++;
    }
    while(1);
}
...
truncate -s 32K os.img

# --- CALISTIR (PORTU TEMIZLEYEREK) ---
pkill -9 qemu-system-i386
echo "Sistem baslatiliyor... VNC Adresi: 127.0.0.1:5999"
qemu-system-i386 -drive format=raw,file=os.img -vnc :99 &
